	...
	protected void generateMethodSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				cstmResultSummary(suite, testContext.getFailedConfigurations(),
						testName, "failed", " (configuration methods)");
				cstmResultSummary(suite, testContext.getFailedTests(), testName,
						"failed", "");
				cstmResultSummary(suite, testContext.getSkippedConfigurations(),
						testName, "skipped", " (configuration methods)");
				cstmResultSummary(suite, testContext.getSkippedTests(), testName,
						"skipped", "");
				cstmResultSummary(suite, testContext.getPassedTests(), testName,
						"passed", "");
				testIndex++;
			}
		}
	...
	private void generateForResult(ITestResult ans, ITestNGMethod method,
		...
		if (hasParameters) {
			...
			for (Object p : parameters) {
				m_out.println("<td>"
						+ Utils.escapeHtml(Utils.toString(p, null)) + "</td>");
			}
			m_out.println("</tr>");
		}
